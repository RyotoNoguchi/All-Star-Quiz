...
const server = new Server(http, {
  ...
  allowRequest: (request, callback) =>
    callback(null, request.headers.origin === 'http://localhost:3000'),
});
...
try {
  ...
  for (const client of clients)
    assert.equal(await client.timeout(2000).emitWithAck('probe'), 'ok');
  ...
  try {
    await once(rejected, 'connect_error');
    assert.equal(rejected.connected, false);
  } finally {
    rejected.close();
  }
  console.log(
    'Transport probe passed: two clients, acknowledgements, reconnect, denied origin.'
  );
} finally {
