...
export const QuizButton: FC<Props> = memo(function QuizButton(props) {
  ...
  const buttonClassName = useMemo(() => {
    let baseClasses = 'w-full h-16 text-lg font-semibold transition-all duration-200 border-2';
    
    if (props.showResult) {
      if (props.isSelected && props.isCorrect) {
        baseClasses += ' bg-green-500 hover:bg-green-600 border-green-400 text-white animate-quiz-bounce';
      } else if (props.isSelected && !props.isCorrect) {
        baseClasses += ' bg-red-500 hover:bg-red-600 border-red-400 text-white animate-quiz-shake';
      ...
      if (props.isSelected) {
        baseClasses += ' bg-blue-500 hover:bg-blue-600 border-blue-400 text-white scale-105 animate-quiz-pulse';
      } else {
        baseClasses += ' bg-white/10 hover:bg-white/20 border-white/30 text-white hover:scale-105';
      }
    }
  ...
  return (
    <Button
      ...
      aria-describedby={props.showResult && props.isCorrect ? 'correct-answer' : undefined}
    >
